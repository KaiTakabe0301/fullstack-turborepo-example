...
const createRule = ESLintUtils.RuleCreator(
  () =>
    'https://github.com/your-org/eslint-rules/blob/main/require-use-memo-in-hooks.md'
);
...
function isNewObjectOrArray(node) {
  // "新しく生成された" オブジェクト / 配列
  return (
    node &&
...
function isUseMemoWrapped(node) {
  // useMemo(…) 呼び出しか
  return (
 ...
 * useMemoラッパー文字列を生成
 */
function buildUseMemoWrapper(node, sourceCode) {
...
// rule
// ---------------------------------------------------------------------------
export const requireUseMemoInHooksRule = createRule({
  name: 'require-use-memo-in-hooks',
  meta: {
    type: 'problem',
    docs: {
      description:
        'Enforce wrapping new objects and arrays with useMemo inside custom hooks.',
      recommended: 'error',
    },
    ...
    messages: {
      useMemo:
        'Wrap this object or array with useMemo inside custom hooks.',
    },
    schema: [],
  },
  defaultOptions: [],
      ...
      'FunctionDeclaration, FunctionExpression, ArrowFunctionExpression'(node) {
        ...
        body.body.forEach(statement => {
          ...
          statement.declarations.forEach(decl => {
            ...
            if (!init || isUseMemoWrapped(init) || !isNewObjectOrArray(init)) return;
